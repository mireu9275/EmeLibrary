package kr.eme.library.coroutine

import kr.eme.library.coroutine.SynchronizationContext.ASYNC
import kr.eme.library.coroutine.SynchronizationContext.SYNC
...
fun sync(block: suspend BukkitSchedulerController.() -> Unit): CoroutineTask = getNewTask(main,SYNC,block)
fun async(block: suspend BukkitSchedulerController.() -> Unit): CoroutineTask = getNewTask(main,ASYNC,block)

private fun getNewTask(plugin: Plugin, initialContext: SynchronizationContext, block: suspend BukkitSchedulerController.() -> Unit): CoroutineTask {
    val controller = BukkitSchedulerController(plugin,plugin.server.scheduler)
    val execute: suspend BukkitSchedulerController.() -> Unit = {
        try { start(initialContext); block() }
        finally { cleanup() }
    }
    execute.createCoroutine(receiver = controller, completion = controller).resume(Unit)
